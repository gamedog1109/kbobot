...
# 1. OpenWeather API 키
API_KEY = "9fd6e52a7c66dd82574f4f87cc79e17b"  # 여기에 실제 API 키 입력
...
# 4. 경기 일정 CSV 로딩
schedule_df = pd.read_csv("KBO_2025_May_to_August.csv")  # 파일명 맞게 수정
tomorrow_games = schedule_df[schedule_df["date"] == tomorrow]
...
# 7. 결과 출력
print(f"[{tomorrow} KBO 경기 우천 가능성 안내]\n")
for game in games:
    result = get_weather(game["lat"], game["lon"], game["stadium"])
    print(f"{game['stadium']} ({game['teams']}): {result}")
